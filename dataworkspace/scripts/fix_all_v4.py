...
plt.close()
print('Fig 1 saved (png/pdf/svg)')

# ═══════════════════════════════════════════
# FIGURE 2 — SHAP + Corroboration (wider C, more hspace)
# ═══════════════════════════════════════════
...
gs = fig.add_gridspec(2, 2, height_ratios=[0.40, 0.60], width_ratios=[0.50, 0.50],
...
# Panel B: Per-class top 5 — more vertical space
...
for idx, (cls, color) in enumerate(zip(class_names, class_colors)):
    ...
    ax.set_xlim(0, max(vals_cls)*1.25)
    if idx > 0:
        ax.set_yticklabels([])
    ax.set_xticks([0, 0.1, 0.2])
    ax.tick_params(axis='x', labelsize=7)

fig.text(0.03, 0.445, 'B  Per-Class Top-5 Dipeptide Features (SHAP Importance)',
         fontweight='bold', fontsize=12, ha='left', va='bottom')

# Panel C: Corroboration table — wider, legend closer
...
table.scale(1.3, 1.4)  # wider cells
for j in range(5):
...
axC.legend(handles=legend_elements, loc='lower center', fontsize=8, ncol=3,
           bbox_to_anchor=(0.5, -0.05))  # legend closer to table
...
plt.close()
print('Fig 2 saved (png/pdf/svg)')

# ═══════════════════════════════════════════
# FIGURE 3 — Window Ablation (no annotation box)
# ═══════════════════════════════════════════
