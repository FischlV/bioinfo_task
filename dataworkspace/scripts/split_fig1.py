...
# ═══ Fig 1a: Workflow ONLY ═══
fig, ax = plt.subplots(figsize=(4, 8))
ax.set_xlim(0, 10); ax.set_ylim(-0.5, 12.5); ax.axis('off')
...
for i, (s, clr) in enumerate(zip(steps, colors)):
    ...
    ax.add_patch(rect)
    ax.text(5, y, s, ha='center', va='center', fontsize=11, fontweight='bold')
    if i < 4:
        ax.annotate('', xy=(5, y - 1.55), xytext=(5, y - 0.85), arrowprops=dict(arrowstyle='->', color='gray', lw=2, shrinkA=0, shrinkB=0))
ax.set_title('Study Workflow', fontweight='bold', fontsize=13)
for ext in ['png','pdf','svg']:
    fig.savefig(f'{FIGS}/fig1a_workflow.{ext}', dpi=300, bbox_inches='tight')
plt.close()
print('Fig 1a saved')
...
sns.heatmap(matrix, annot=np.array(annot), fmt='', cmap='RdYlGn',
            ...
            vmin=30, vmax=66, linewidths=0.5, linecolor='white',
...
ax.set_title('Model x Encoding Accuracy (%, mean of 3 seeds)', fontweight='bold', fontsize=13)
plt.tight_layout()
for ext in ['png','pdf','svg']:
    fig.savefig(f'{FIGS}/fig1b_heatmap.{ext}', dpi=300, bbox_inches='tight')
plt.close()
print('Fig 1b saved')
print('Done!')
