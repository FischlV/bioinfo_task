...
# Build matrix (as percentages 0-100)
matrix = np.zeros((3, 7))
annot = [['']*7 for _ in range(3)]
for i, m in enumerate(model_order):
    for j, e in enumerate(enc_order):
        mean_val = np.mean(all_acc[e][m]) * 100  # to percentage
        matrix[i, j] = mean_val
...
gs_b = gs[1, 0].subgridspec(1, 5, wspace=0.35)
...
for idx, (cls, color) in enumerate(zip(class_names, class_colors)):
    ...
    if idx > 0:
        ax.set_yticklabels([])
    # Clean x-axis: just a few ticks
    ax.set_xticks([0, 0.1, 0.2])
    ax.tick_params(axis='x', labelsize=7)
    

fig.text(0.04, 0.47, 'B  Per-Class Top-5 Dipeptide Features (SHAP Importance)',
...
col_labels = ['Dipeptide', 'Class', 'Ratio', 'p-adj', 'Verdict w/ Literature']
cell_colors = []
